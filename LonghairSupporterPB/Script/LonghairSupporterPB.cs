...
public class LonghairSupporterPB : MonoBehaviour {
    ...
    [Header("Option")]
    [Tooltip("髪のPhysBoneをクリアして再設定する")]
    public bool physBoneReset;
    ...
    [HairTypeArrayPB()]
    public VRCPhysBone[] physBoneModels = new VRCPhysBone[Enum.GetValues(typeof(HairType)).Length];
    ...
    private void ArrayFix()
    {
        ...
        if (physBoneModels.Length != size)
        {
            Array.Resize(ref physBoneModels, size);
        }
